...
/**
 * @brief Funcion get_users de la api encargada de listar todos los usuarios existentes.
 * 
 * @param request request user
 * @param response responde hacia el usuario
 * @param user_data 
 * @return int 
 */
...
/**
 * @brief Funcion encargada POST de la api encargada de generar un nuevo usuario en el sistema mediante user y password
 * 
...
int callback_post_users(const struct _u_request * request, struct _u_response * response, void * user_data){
  ...
  sprintf(command, "sudo useradd -m -p $(perl -e 'print crypt($ARGV[0], \"password\")' '%s') %s", password, user) ;
...
/**
 * @brief Funcion que se encargar de obtener todos los usuarios en formato json
 * 
 * @return json_t* 
 */
...
/**
 * @brief Funcion encargada de escribir el log
 * 
 * @param timestamp Timestamp
 * @param name_service Tipo de servicio
